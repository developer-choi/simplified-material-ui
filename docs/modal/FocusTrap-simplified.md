...
### 제거된 기능
1. ❌ disableAutoFocus, disableEnforceFocus, disableRestoreFocus props
2. ❌ getTabbable, isEnabled props
3. ❌ interval 주기적 체크 (브라우저 버그 우회)
4. ❌ 라디오 버튼 특수 처리
5. ❌ tabIndex 정렬 로직 (복잡한 알고리즘)
6. ❌ React 이벤트 트리 추적
7. ❌ useForkRef, getReactElementRef 등 유틸리티
...
1. **브라우저 제한** - 최신 Chrome, Firefox, Safari만 지원
2. **tabIndex 정렬 미지원** - DOM 순서대로만 동작
3. **라디오 버튼 미지원** - 포커스 로빙 없음
4. **Props 제거** - 모든 커스터마이징 불가
5. **interval 없음** - 일부 브라우저에서 포커스 복원 실패 가능
